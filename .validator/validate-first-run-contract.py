...
def main() -> int:
    ...
    if "Choose your fork destination" not in first_run or "Suggested fork" not in first_run:
        fail("fork destination input is missing visible instructions and a default")
    if "already exists but is not a fork of Adaetum/Adaetum" not in first_run:
        fail("first-run does not recover from a same-name non-fork repository")
    if "first_run_find_available_fork_destination" not in first_run:
        fail("first-run does not search for an available fork name after a collision")
    if 'candidate="${owner}/Adaetum-cluster-${suffix}"' not in first_run:
        fail("fork collision recovery can repeat the same occupied suggestion")
    ...
    if 'existing="$(gh api "repos/${repository}"' in first_run:
        fail("fork lookup mistakes GitHub's 404 response body for an existing repository")
    ...
    if "Account API token" not in first_run or "Account API Tokens: Edit" not in first_run:
    ...
    if "Connectivity Directory: Admin" not in first_run:
        fail("Cloudflare token guidance uses the retired Tunnel permission label")
    if "Workers Scripts: Edit" not in first_run or "Workers Routes: Edit" not in first_run:
