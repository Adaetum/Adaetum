...
# Phase 50 establishes the GitOps control pair. It creates the bootstrap
# Gitea/Argo CD path and proves the cluster can reconcile its own repository.
# It may create bootstrap resources, but it does not remove local bootstrap
# authority; Phase 60 owns that irreversible handoff.
...
phase50_log_root="${BOOTSTRAP_PHASE_LOG_DIR:-/var/log/bootstrap}"
# Phase controls intentionally default to install. Operators can rerun a
# narrower handoff or post-handoff path without pretending it is a clean first
# install.
PHASE50_LOG_FILE="${PHASE50_LOG_FILE:-${phase50_log_root}/phase50.log}"
PHASE50_ARGOCD_DEBUG="${PHASE50_ARGOCD_DEBUG:-1}"
PHASE50_ARGOCD_DEBUG_LOG="${PHASE50_ARGOCD_DEBUG_LOG:-${PHASE50_LOG_FILE}}"
PHASE50_RECONCILE_ONLY="${PHASE50_RECONCILE_ONLY:-0}"
PHASE50_MODE="${PHASE50_MODE:-install}"
BOOTSTRAP_GITEA_GOLDEN_PATH_REQUIRED="${BOOTSTRAP_GITEA_GOLDEN_PATH_REQUIRED:-1}"
PHASE50_GITEA_ROLLOUT_TIMEOUT="${PHASE50_GITEA_ROLLOUT_TIMEOUT:-600s}"
PHASE50_GITEA_PROGRESS_DEADLINE_SECONDS="${PHASE50_GITEA_PROGRESS_DEADLINE_SECONDS:-1800}"
PHASE50_GITEA_ROLLOUT_ATTEMPTS="${PHASE50_GITEA_ROLLOUT_ATTEMPTS:-2}"
PHASE50_GITEA_ROLLOUT_RESTART_ON_FAIL="${PHASE50_GITEA_ROLLOUT_RESTART_ON_FAIL:-1}"
PHASE50_GITEA_DEBUG_LOG="${PHASE50_GITEA_DEBUG_LOG:-${PHASE50_LOG_FILE}}"
PHASE50_GITEA_POSTGRESQL_SECRET_ATTEMPTS="${PHASE50_GITEA_POSTGRESQL_SECRET_ATTEMPTS:-60}"
PHASE50_GITEA_POSTGRESQL_SECRET_DELAY="${PHASE50_GITEA_POSTGRESQL_SECRET_DELAY:-5}"
PHASE50_CLOUDFLARED_ROLLOUT_TIMEOUT="${PHASE50_CLOUDFLARED_ROLLOUT_TIMEOUT:-300s}"
PHASE50_CLOUDFLARED_ROLLOUT_ATTEMPTS="${PHASE50_CLOUDFLARED_ROLLOUT_ATTEMPTS:-2}"
PHASE50_CLOUDFLARED_ROLLOUT_RESTART_ON_FAIL="${PHASE50_CLOUDFLARED_ROLLOUT_RESTART_ON_FAIL:-1}"
PHASE50_CLOUDFLARED_DEBUG_LOG="${PHASE50_CLOUDFLARED_DEBUG_LOG:-${PHASE50_LOG_FILE}}"
PHASE50_RANCHER_ROLLOUT_TIMEOUT="${PHASE50_RANCHER_ROLLOUT_TIMEOUT:-600s}"
PHASE50_RANCHER_DEBUG_LOG="${PHASE50_RANCHER_DEBUG_LOG:-${PHASE50_LOG_FILE}}"
PHASE50_RANCHER_DESIRED_REPLICAS="${PHASE50_RANCHER_DESIRED_REPLICAS:-auto}"
PHASE50_RANCHER_PROGRESS_DEADLINE_SECONDS="${PHASE50_RANCHER_PROGRESS_DEADLINE_SECONDS:-1800}"
...
PHASE50_DOMAIN_CHECK_DELAY="${PHASE50_DOMAIN_CHECK_DELAY:-10}"
PHASE50_DOMAIN_HTTP_SUCCESS_REGEX="${PHASE50_DOMAIN_HTTP_SUCCESS_REGEX:-^(2[0-9][0-9]|3[0-9][0-9]|401|403)$}"
PHASE50_INGRESS_FRONTDOOR_WAIT_TIMEOUT="${PHASE50_INGRESS_FRONTDOOR_WAIT_TIMEOUT:-300}"
PHASE50_INGRESS_FRONTDOOR_WAIT_DELAY="${PHASE50_INGRESS_FRONTDOOR_WAIT_DELAY:-5}"
...
case "${PHASE50_MODE}" in
  install|handoff|post-handoff|full)
    ;;
  *)
    echo "invalid PHASE50_MODE=${PHASE50_MODE}; expected install, handoff, post-handoff, or full" >&2
    exit 1
    ;;
esac

# Post-handoff checks report evidence but must not recreate bootstrap state.
PHASE50_WARNING_ONLY=0
if [[ "${PHASE50_MODE}" == "post-handoff" ]]; then
  PHASE50_WARNING_ONLY=1
fi
BOOTSTRAP_DIAG_PHASE="phase50"
BOOTSTRAP_DIAG_LOG_PATH="${PHASE50_LOG_FILE:-${BUNDLE_BOOTSTRAP_LOG_FILE:-}}"
bootstrap_diag_init
phase50_start_ts="$(date +%s)"
phase50_last_error_cmd=""
phase50_last_error_line=""

if [[ -z "${BUNDLE_BOOTSTRAP_LOG_FILE:-}" ]]; then
  mkdir -p "$(dirname "${PHASE50_LOG_FILE}")"
  exec >>"${PHASE50_LOG_FILE}" 2>&1
fi
...
if ! kubectl_bin="$(bootstrap_control_pair_resolve_kubectl)"; then
  echo "kubectl not found; Phase 50 requires kubectl access to the cluster." >&2
  exit 1
fi
...
github_app_refresh_repo_auth() {
  ...
  echo "[phase50] minting GitHub App installation token for source repo access"
  token="$(github_app_mint_token || true)"
  if [[ -z "${token}" ]]; then
    echo "[phase50] WARNING: GitHub App token mint failed; source repo auth will rely on explicit seed token envs" >&2
    return 0
  fi
...
phase50_log_source_repo_auth() {
  ...
  echo "[phase50] source repo auth summary:"
  echo "[phase50]   url=${source_repo_url:-<empty>}"
  echo "[phase50]   username=${source_repo_username:-<empty>} (source=${username_source})"
  echo "[phase50]   token_present=${token_present} kind=${token_kind} length=${token_length} source=${token_source}"
}
...
rehydrate_ingress_vip_config() {
  ...
  if [[ -n "${INGRESS_INTERNAL_VIP:-}" || -n "${INGRESS_EXTERNAL_VIP:-}" ]]; then
    echo "[phase50] hydrated ingress VIP config from ${config_ns}/${config_name}"
  fi
}


echo "[phase50] repo: ${repo_root}"
echo "[phase50] checking cluster connectivity"
bootstrap_diag_record \
  ...
  "operation=run-start" \
  "severity=info" \
  "summary=phase50 ${PHASE50_MODE} starting" \
  "log_path=${BOOTSTRAP_DIAG_LOG_PATH}" \
  "post_handoff=$([[ "${PHASE50_MODE}" == "post-handoff" ]] && echo true || echo false)"
...
echo "[phase50] kube service domain: ${KUBE_SERVICE_DOMAIN}"
...
ensure_authentik_secret() {
  ...
  if [[ -z "${secret_key}" || -z "${postgresql_password}" || -z "${bootstrap_password}" || -z "${bootstrap_token}" ]]; then
    echo "[phase50] Authentik bootstrap credentials are incomplete" >&2
    return 1
  fi
  "${kubectl_bin}" create namespace authentik --dry-run=client -o yaml | "${kubectl_bin}" apply -f - >/dev/null
  # Runtime values are promoted to OpenBao below. CSI creates application
  # copies at pod start; the database coordinator owns the active chart Secret.
  # Phase 50 must never overwrite either from bootstrap inputs on a rerun.
  ...
  if ! "${kubectl_bin}" -n authentik get secret authentik-postgresql >/dev/null 2>&1; then
    echo "[phase50] Authentik active PostgreSQL Secret is absent after OpenBao handoff; refusing bootstrap fallback" >&2
    return 1
  fi
}
...
EOF
  ...
  echo "[phase50] failed waiting for Headlamp admin service-account token" >&2
  return 1
}
...
ensure_gitea_runtime_host_aliases() {
  ...
  if [[ -z "${gitea_ip}" ]]; then
    echo "[phase50] warning: unable to determine Gitea service IP for runtime host aliases" >&2
    return 0
  fi
...
PY
  ...
  mv "${tmp}" "${hosts_path}"
  echo "[phase50] ensured runtime host aliases for ${canonical_host:-<none>} ${local_host:-<none>} via ${gitea_ip}"
}
...
ensure_ansible_runner_registry_runtime() {
  ...
  case "${registry_host}" in
    ...
    *)
      if [[ -n "${bootstrap_registry_ip}" ]]; then
        ...
        echo "[phase50] warning: unable to resolve bootstrap Gitea registry endpoint for ${registry_host}; leaving runtime registry config unchanged" >&2
        return 0
      fi
      ;;
  esac
  ...
  if [[ -f "${registries_path}" ]]; then
    ...
    if [[ -n "${current}" && "${current}" != *"${managed_marker}"* && "${current}" != *"\"${registry_host}\""* ]]; then
      echo "[phase50] warning: existing ${registries_path} is unmanaged; not overwriting it automatically" >&2
      return 0
    fi
  fi
...
EOF
  ...
  echo "[phase50] ansible-runner registry runtime config already present for ${registry_host}"
  return 0
  fi
  ...
  echo "[phase50] wrote ${registries_path} for bootstrap registry ${registry_host} via ${mirror_endpoint}"

  if command -v systemctl >/dev/null 2>&1; then
    echo "[phase50] restarting rke2-server to pick up registry config"
    systemctl restart rke2-server
    if ! wait_for_kube_api_ready 60 5; then
      echo "[phase50] Kubernetes API did not return after restarting rke2-server" >&2
      return 1
    fi
    echo "[phase50] rke2-server restarted and Kubernetes API is reachable again"
  else
    echo "[phase50] warning: systemctl not found; restart rke2-server manually to apply ${registries_path}" >&2
  fi
}
...
PY
  ...
  if [[ -z "${selector_yaml}" ]]; then
    echo "[phase50] WARNING: unable to derive selector for bootstrap Gitea access service from ${GITEA_INTERNAL_SERVICE_NAME}" >&2
    return 1
  fi
...
${selector_yaml}
  ports:
    ...
    - name: http-alt
      ...
      targetPort: 3000
EOF
  echo "[phase50] ensured bootstrap Gitea access service gitea-bootstrap-access"
}
...
refresh_gitea_internal_service_url() {
  ...
  if [[ -n "${detected}" ]]; then
    ...
    if [[ -n "${bootstrap_host}" ]]; then
      echo "[phase50] detected Gitea internal service -> ${GITEA_INTERNAL_SERVICE_NAME} (${GITEA_INTERNAL_SERVICE_HOST}); bootstrap registry endpoint ${bootstrap_host}:3000"
    else
      echo "[phase50] detected Gitea internal service -> ${GITEA_INTERNAL_SERVICE_NAME} (${GITEA_INTERNAL_SERVICE_HOST})"
    fi
  else
    echo "[phase50] WARNING: unable to detect Gitea internal HTTP service; keeping ${GITEA_INTERNAL_SERVICE_NAME}" >&2
  fi
}
...
ensure_ansible_runner_pull_secret() {
  ...
  if [[ -z "${registry_token}" ]]; then
    echo "[phase50] failed to determine Gitea registry token for ansible-runner pulls" >&2
    return 1
  fi
  ...
  if [[ "${push_host}" != "${registry_host}" ]]; then
    echo "[phase50] ensured ansible-runner imagePullSecret ${secret_name} for ${registry_host} and internal push host ${push_host}"
  else
    echo "[phase50] ensured ansible-runner imagePullSecret ${secret_name} for ${registry_host}"
  fi
}
...
publish_ansible_runner_image() {
  ...
  local manifest=""
  local timeout_secs="${PHASE50_ANSIBLE_RUNNER_BUILD_TIMEOUT_SECONDS:-900}"
  local wait_rc=0
  local elapsed=0
  local wait_step="${PHASE50_ANSIBLE_RUNNER_BUILD_STATUS_DELAY_SECONDS:-15}"
  local succeeded=""
  local failed=""
  local active=""
  ...
  if [[ "${push_image}" != "${runner_image}" ]] && container_registry_image_exists "${push_image}"; then
    echo "[phase50] ansible-runner image already present on internal registry push host: ${push_image}"
    return 0
  fi
  if [[ "${push_image}" == "${runner_image}" ]] && container_registry_image_exists "${runner_image}"; then
    echo "[phase50] ansible-runner image already present: ${runner_image}"
    return 0
  fi

  if [[ ! -f "${repo_root}/ansible/Dockerfile" ]]; then
    echo "[phase50] missing ${repo_root}/ansible/Dockerfile; cannot publish ansible-runner image" >&2
    return 1
  fi
...
PY
      ...
      if [[ -n "${host_alias_block}" ]]; then
        echo "[phase50] kaniko will resolve Gitea token-service hostnames via service IP ${gitea_service_ip}"
      fi
    fi
  fi
...
EOF
  ...
  echo "[phase50] building ansible-runner image with kaniko job ${job_name}"
  echo "[phase50] waiting up to ${timeout_secs}s for kaniko build/push to ${push_image}"
  ...
  while (( elapsed < timeout_secs )); do
    ...
    if (( elapsed > 0 )) && (( elapsed % 60 == 0 )); then
      echo "[phase50] waiting for ${job_name}: elapsed=${elapsed}s active=${active:-0} succeeded=${succeeded:-0} failed=${failed:-0}"
    fi
    sleep "${wait_step}"
  ...
  if [[ "${wait_rc}" != "0" ]] && container_registry_image_exists "${push_image}"; then
    echo "[phase50] kaniko job no longer visible, but ansible-runner image is now present on the internal push host: ${push_image}"
    wait_rc=0
  fi

  if [[ "${wait_rc}" != "0" ]]; then
    echo "[phase50] ansible-runner image build job failed or timed out; logs follow" >&2
  ...
  if ! container_registry_image_exists "${push_image}"; then
    echo "[phase50] ansible-runner image still missing after build: ${push_image}" >&2
    return 1
  fi

  if [[ "${push_image}" != "${runner_image}" ]] && ! container_registry_image_exists "${runner_image}"; then
    echo "[phase50] published ansible-runner image to internal registry endpoint ${push_image}; canonical registry hostname ${runner_image} is not readable yet (expected during ingress/DNS convergence)"
    return 0
  fi

  echo "[phase50] published ansible-runner image ${runner_image}"
}
...
ensure_external_dns_cloudflare_secret() {
  ...
  # CSI owns the Kubernetes delivery Secret. Phase 50 may seed OpenBao during
  # the handoff, but must never recreate a workload credential from bootstrap.
  echo "[phase50] seeding OpenBao ingress/external-dns from ${token_source}"
...
apply_ansible_runner_deployment() {
  ...
  if [[ ! -f "${manifest_path}" ]]; then
    echo "[phase50] skip: missing ${manifest_path}"
    return 0
  fi
...
PY
  ...
  echo "[phase50] applied ansible-runner deployment with image ${runner_image}"
}
...
apply_external_dns_manifests() {
  ...
  if [[ ! -f "${deployment_path}" || ! -f "${rbac_path}" ]]; then
    echo "[phase50] missing external-dns manifest inputs under ${base_dir}" >&2
    return 1
  fi
...
PY
  ...
  echo "[phase50] applied external-dns manifests with domain filter ${CLUSTER_DOMAIN}"
}
...
apply_kube_vip_manifests() {
  ...
  echo "[phase50] applying pods/ingress/kube-vip"
  run_or_fail \
    "failed applying pods/ingress/kube-vip" \
    "${kubectl_bin}" -n kube-system apply -k "${kube_vip_dir}"

  echo "[phase50] waiting for kube-vip daemonset rollout"
  run_or_fail \
    "kube-vip daemonset did not become ready" \
    "${kubectl_bin}" -n kube-system rollout status daemonset/kube-vip-ds --timeout="${PHASE50_KUBE_VIP_ROLLOUT_TIMEOUT:-180s}"

  echo "[phase50] applying pods/ingress/ingress-vip"
  run_or_fail \
    "failed applying pods/ingress/ingress-vip" \
    "${kubectl_bin}" -n ingress apply -k "${ingress_vip_dir}"

  echo "[phase50] ensuring ingress-vip cronjob exists"
  run_or_fail \
...
ensure_routing_frontdoor_vip() {
  ...
  echo "[phase50] setting ${routing_service_ref} type=${desired_service_type}"
  run_or_fail \
  ...
  if [[ -n "${existing_lb_class}" ]]; then
    echo "[phase50] warning: ${routing_service_ref} already has loadBalancerClass=${existing_lb_class}; Phase 50 will not mutate it" >&2
  fi

  if [[ -n "${desired_vip}" ]]; then
    echo "[phase50] pinning ${routing_service_ref} vip=${desired_vip}"
    run_or_fail \
    ...
    echo "[phase50] requesting ${routing_service_ref} vip via DHCP (0.0.0.0)"
    run_or_fail \
    ...
    concrete_vip="$(wait_for_concrete_ingress_vip "${routing_service_ns}" "${routing_service_name}" "${PHASE50_INGRESS_VIP_WAIT_TIMEOUT:-180}" "${PHASE50_INGRESS_VIP_WAIT_DELAY:-5}" || true)"
    if [[ -z "${concrete_vip}" ]]; then
      fail_local_requirement "failed waiting for concrete ingress VIP assignment on ${routing_service_ref}"
    fi
    echo "[phase50] ${routing_service_ref} assigned vip=${concrete_vip}"
  ...
  if [[ -z "${vip_eff}" ]]; then
    echo "[phase50] warning: unable to determine ingress VIP from ${routing_service_ref}; skipping strict VIP/DNS IP match for local domains"
...
reconcile_argocd_bootstrap_repo() {
  ...
  if [[ -z "${repo_url}" ]]; then
    echo "[phase50] Argo CD bootstrap repo URL is empty; refusing reconciliation" >&2
    return 1
  fi

  echo "[phase50] reconciling Argo CD bootstrap repo -> ${repo_url}"
...
EOF
  ...
  if [[ ! -f "${bootstrap_dir}/app-of-apps.yaml" || ! -f "${bootstrap_dir}/applicationset.yaml" ]]; then
    echo "[phase50] bootstrap Argo CD manifests missing under ${bootstrap_dir}" >&2
    return 1
  fi
...
reconcile_pre_openbao_application_repo() {
  ...
  if [[ -z "${repo_url}" ]]; then
    echo "[phase50] pre-openbao repo reconciliation skipped: repo URL missing" >&2
    return 1
  fi

  if ! "${kubectl_bin}" -n argocd get application platform-pre-openbao >/dev/null 2>&1; then
    echo "[phase50] pre-openbao application not present yet; skipping repo handoff"
    return 0
  fi
...
verify_gitea_gitops_handoff() {
  ...
  if [[ -z "${repo_url}" ]]; then
    echo "[phase50] GitOps handoff verification failed: repo URL missing" >&2
    return 1
  fi
  ...
  if [[ "${app_repo_url}" != "${repo_url}" ]]; then
    echo "[phase50] GitOps handoff verification failed: app-of-apps repoURL=${app_repo_url:-<empty>} expected=${repo_url}" >&2
    return 1
  fi
  if [[ "${set_repo_url}" != "${repo_url}" ]]; then
    echo "[phase50] GitOps handoff verification failed: applicationset repoURL=${set_repo_url:-<empty>} expected=${repo_url}" >&2
    return 1
  fi
  if [[ "${secret_repo_url}" != "${repo_url}" ]]; then
    echo "[phase50] GitOps handoff verification failed: argocd-repository-bootstrap url=${secret_repo_url:-<empty>} expected=${repo_url}" >&2
    return 1
  fi
  if [[ -n "${pre_openbao_repo_url}" && "${pre_openbao_repo_url}" != "${repo_url}" ]]; then
    echo "[phase50] GitOps handoff verification failed: platform-pre-openbao repoURL=${pre_openbao_repo_url} expected=${repo_url}" >&2
    return 1
  fi
  if [[ -n "${pre_openbao_repo_path}" && "${pre_openbao_repo_path}" != "${pre_openbao_path}" ]]; then
    echo "[phase50] GitOps handoff verification failed: platform-pre-openbao path=${pre_openbao_repo_path} expected=${pre_openbao_path}" >&2
    return 1
  fi
  ...
  if [[ -n "${repo_token}" && -n "${repo_username}" ]] && printf '%s' "${repo_url}" | grep -qE '^https?://'; then
    if ! gitea_repo_has_readable_refs_via_local_portforward "${repo_url}" "${repo_username}" "${repo_token}"; then
      echo "[phase50] GitOps handoff verification failed: gitea repo has no readable refs via local Gitea path for ${repo_url}" >&2
      return 1
    fi
...
SH
    ...
    if [[ -z "${refs_output}" ]]; then
      echo "[phase50] WARNING: gitea repo refs are readable via local Gitea path but not yet via canonical repo URL ${repo_url}; continuing (likely DNS/ingress convergence)" >&2
    fi
  fi

  echo "[phase50] verified GitOps handoff: Gitea repo is seeded and Argo CD points at ${repo_url} (${repo_branch})"
}
...
ensure_gitea_admin_secret() {
  ...
  if [[ -z "${admin_password}" ]]; then
    echo "[phase50] Gitea admin password missing; cannot bootstrap Gitea via Argo" >&2
    return 1
  fi

  # The Gitea CSI mount creates the chart-required Secret from OpenBao during
  # pod setup. Do not create a post-handoff administrator copy here.
...
ensure_gitea_actions_runner() {
  ...
  if [[ -z "${runner_token}" ]]; then
    echo "[phase50] Gitea runner token missing; cannot deploy actions runner" >&2
    return 1
  fi
  if [[ -z "${instance_url}" ]]; then
    echo "[phase50] Gitea instance URL missing; cannot deploy actions runner" >&2
    return 1
  fi
...
EOF
  ...
  echo "[phase50] ensured Gitea actions runner deployment with labels ${runner_labels}"
}
...
seed_gitea_bootstrap_repo() {
  ...
  if [[ -z "${admin_password}" || -z "${canonical_url}" ]]; then
    echo "[phase50] missing inputs for Gitea seed helper" >&2
    return 1
  fi
  ...
  if [[ -z "${pod}" ]]; then
    echo "[phase50] no ready Gitea pod found for repo seed" >&2
    return 1
  fi
  ...
  if [[ -z "${admin_token}" ]]; then
    echo "[phase50] failed to mint bootstrap Gitea API token" >&2
    return 1
  fi
...
PY
    ...
    case "${skip_remote_when_present}" in
      ...
      *)
        if [[ "${repo_has_refs}" == "1" ]]; then
          echo "[phase50] target repo already exists in Gitea with refs; skipping remote reseed"
  ...
  if [[ "${seeded_from_remote}" != "1" && "${trust_gitea_repo}" != "0" && "${trust_gitea_repo}" != "false" && "${trust_gitea_repo}" != "FALSE" && "${trust_gitea_repo}" != "no" && "${trust_gitea_repo}" != "NO" && "${trust_gitea_repo}" != "off" && "${trust_gitea_repo}" != "OFF" ]]; then
    if [[ "${repo_exists}" == "1" && "${repo_has_refs}" == "1" ]]; then
      echo "[phase50] trusting existing Gitea bootstrap repo with refs; skipping remote reseed"
      seeded_from_remote="1"
  ...
  if [[ -n "${source_repo_url}" ]]; then
    ...
    if [[ -n "${source_repo_token}" ]] && printf '%s' "${source_repo_url}" | grep -qE '^https://github.com/'; then
      ...
      phase50_log_source_repo_auth "${source_repo_url}" "${source_repo_username}" "${source_repo_token}" "${source_repo_token_source}" "${source_repo_username_source}"
      echo "[phase50] ERROR: GitHub source repo auth missing for ${source_repo_url}" >&2
      echo "[phase50] set GITEA_SEED_SOURCE_TOKEN or provide GITHUB_APP_ID, GITHUB_APP_INSTALLATION_ID, and GITHUB_APP_PRIVATE_KEY_B64 in the runtime payload" >&2
      return 1
    fi
  fi
...
SH
    ...
    if [[ "${git_ls_remote_rc}" == "0" ]]; then
      ...
      echo "[phase50] seeding Gitea repo from remote source ${source_repo_url}"
      if [[ -n "${source_repo_token}" && -n "${source_repo_username}" ]] && printf '%s' "${clone_url}" | grep -qE '^https://'; then
        GIT_ASKPASS="${source_askpass}" \
          GIT_ASKPASS_USERNAME="${source_repo_username}" \
          GIT_ASKPASS_PASSWORD="${source_repo_token}" \
      ...
      if [[ "${git_clone_rc}" == "0" ]]; then
        ...
        echo "[phase50] ERROR: remote repo clone failed for ${source_repo_url}" >&2
        return 1
      fi
    else
      echo "[phase50] ERROR: remote repo probe failed for ${source_repo_url}" >&2
  ...
  if [[ "${seeded_from_remote}" != "1" ]]; then
    echo "[phase50] no usable Gitea repo or remote source repo for seed; offline fallback is disabled" >&2
    return 1
  fi
...
SH
    ...
    if [[ "${git_push_rc}" != "0" ]]; then
      echo "[phase50] ERROR: failed to mirror-push seeded repo into Gitea" >&2
      return 1
    fi
    pushed_refs="$(
      GIT_ASKPASS="${askpass}" \
    ...
    if [[ -z "${pushed_refs}" ]]; then
      echo "[phase50] ERROR: mirrored repo push completed but Gitea still exposes no readable refs" >&2
      return 1
    fi
  fi
  ...
  if [[ -n "${clone_url}" && "${seeded_from_remote}" == "1" ]]; then
    echo "[phase50] Gitea repo seeded -> ${gitea_git_url}"
  else
    echo "[phase50] Gitea repo selected -> ${gitea_git_url}"
  fi
  echo "[phase50] Argo CD repo source -> ${argocd_git_url}"
}

fail_local_requirement() {
  if [[ "${PHASE50_WARNING_ONLY}" == "1" ]]; then
    echo "[phase50] WARNING: $*" >&2
    bootstrap_diag_record \
      "phase=phase50" \
      "step=${PHASE50_CURRENT_STEP:-phase50}" \
      "component=${PHASE50_CURRENT_COMPONENT:-phase50}" \
...
run_or_fail() {
  ...
  shift
  PHASE50_CURRENT_STEP="${desc}"
  if ! "$@"; then
    fail_local_requirement "${desc}"
  fi
}

phase50_component_debug_dump() {
...
phase50_run_start_recorded=0
phase50_exit_trap() {
  local rc=$?
  local end_ts duration
  end_ts="$(date +%s)"
  duration="$((end_ts - phase50_start_ts))"
  if [[ "${rc}" -eq 0 ]]; then
    bootstrap_diag_record \
      "phase=phase50" \
      "step=${PHASE50_MODE}" \
      ...
      "summary=phase50 ${PHASE50_MODE} complete" \
      "log_path=${BOOTSTRAP_DIAG_LOG_PATH}" \
      "post_handoff=$([[ "${PHASE50_MODE}" == "post-handoff" ]] && echo true || echo false)"
  else
    bootstrap_diag_record \
      "phase=phase50" \
      "step=${PHASE50_CURRENT_STEP:-${PHASE50_MODE}}" \
      "component=${PHASE50_CURRENT_COMPONENT:-phase50}" \
      "operation=run-failed" \
      "severity=error" \
      "exit_code=${rc}" \
      "duration_seconds=${duration}" \
      "failure_kind=$(bootstrap_diag_classify_failure_kind "${phase50_last_error_cmd}")" \
      "summary=phase50 ${PHASE50_MODE} failed at line ${phase50_last_error_line:-unknown}: ${phase50_last_error_cmd:-unknown}" \
      "log_path=${BOOTSTRAP_DIAG_LOG_PATH}" \
      "post_handoff=$([[ "${PHASE50_MODE}" == "post-handoff" ]] && echo true || echo false)"
  fi
  exit "${rc}"
}
trap 'phase50_exit_trap' EXIT
trap 'phase50_last_error_line="${BASH_LINENO[0]:-unknown}"; phase50_last_error_cmd="${BASH_COMMAND:-unknown}"' ERR
...
gitea_debug_dump() {
  ...
  mkdir -p "$(dirname "${PHASE50_GITEA_DEBUG_LOG}")" 2>/dev/null || true
  {
    echo ""
    echo "===== phase50 gitea debug @ ${ts} (${reason}) ====="
    ...
    echo "===== end phase50 gitea debug ====="
  } >>"${PHASE50_GITEA_DEBUG_LOG}" 2>&1
  bootstrap_diag_record_file_event \
    "gitea" \
    "gitea-debug" \
    "captured gitea diagnostics (${reason})" \
    "${PHASE50_GITEA_DEBUG_LOG}" \
    "warning" \
    "rollout"
}
...
cloudflared_debug_dump() {
  ...
  mkdir -p "$(dirname "${PHASE50_CLOUDFLARED_DEBUG_LOG}")" 2>/dev/null || true
  {
    echo ""
    echo "===== phase50 cloudflared debug @ ${ts} (${reason}) ====="
    ...
    echo "===== end phase50 cloudflared debug ====="
  } >>"${PHASE50_CLOUDFLARED_DEBUG_LOG}" 2>&1
  bootstrap_diag_record_file_event \
    "cloudflared" \
    "cloudflared-debug" \
    "captured cloudflared diagnostics (${reason})" \
    "${PHASE50_CLOUDFLARED_DEBUG_LOG}" \
    "warning" \
    "rollout"
}
...
rancher_debug_dump() {
  ...
  mkdir -p "$(dirname "${PHASE50_RANCHER_DEBUG_LOG}")" 2>/dev/null || true
  {
    echo ""
    echo "===== phase50 rancher debug @ ${ts} (${reason}) ====="
    ...
    echo "===== end phase50 rancher debug ====="
  } >>"${PHASE50_RANCHER_DEBUG_LOG}" 2>&1
  bootstrap_diag_record_file_event \
    "rancher" \
    "rancher-debug" \
    "captured rancher diagnostics (${reason})" \
    "${PHASE50_RANCHER_DEBUG_LOG}" \
    "warning" \
    "rollout"
}

wait_for_gitea_rollout() {
  local attempts="${PHASE50_GITEA_ROLLOUT_ATTEMPTS}"
  local attempt=1
  while (( attempt <= attempts )); do
    if "${kubectl_bin}" -n gitea rollout status deployment/gitea --timeout="${PHASE50_GITEA_ROLLOUT_TIMEOUT}"; then
      return 0
    fi

    echo "[phase50] warning: gitea rollout attempt ${attempt}/${attempts} failed"
    gitea_debug_dump "rollout-attempt-${attempt}-failed"

    if (( attempt < attempts )); then
      if [[ "${PHASE50_GITEA_ROLLOUT_RESTART_ON_FAIL}" == "1" || "${PHASE50_GITEA_ROLLOUT_RESTART_ON_FAIL}" == "true" ]]; then
        echo "[phase50] warning: restarting deployment/gitea before retry"
...
wait_for_cloudflared_rollout() {
  local attempts="${PHASE50_CLOUDFLARED_ROLLOUT_ATTEMPTS}"
  local attempt=1
  while (( attempt <= attempts )); do
    if "${kubectl_bin}" -n cloudflared rollout status deployment/cloudflared --timeout="${PHASE50_CLOUDFLARED_ROLLOUT_TIMEOUT}"; then
      return 0
    fi

    echo "[phase50] warning: cloudflared rollout attempt ${attempt}/${attempts} failed"
    cloudflared_debug_dump "rollout-attempt-${attempt}-failed"

    if (( attempt < attempts )); then
      if [[ "${PHASE50_CLOUDFLARED_ROLLOUT_RESTART_ON_FAIL}" == "1" || "${PHASE50_CLOUDFLARED_ROLLOUT_RESTART_ON_FAIL}" == "true" ]]; then
        echo "[phase50] warning: restarting deployment/cloudflared before retry"
...
require_cloudflared_ready() {
  ...
  if ! wait_for_cloudflared_rollout; then
    fail_local_requirement \
      "cloudflared deployment did not become ready within ${PHASE50_CLOUDFLARED_ROLLOUT_TIMEOUT} (attempts=${PHASE50_CLOUDFLARED_ROLLOUT_ATTEMPTS}); see ${PHASE50_CLOUDFLARED_DEBUG_LOG}"
  fi
  echo "[phase50] cloudflared rollout checks passed"
}

detect_rancher_desired_replicas() {
  local requested="${PHASE50_RANCHER_DESIRED_REPLICAS}"
  local ready_count=""
...
tune_rancher_deployment() {
  ...
  echo "[phase50] tuning rancher deployment (replicas=${replicas}, startupFailureThreshold=${PHASE50_RANCHER_STARTUP_FAILURE_THRESHOLD})"
  "${kubectl_bin}" -n cattle-system patch deployment rancher --type=strategic -p "{
    \"spec\": {
      \"replicas\": ${replicas},
      \"progressDeadlineSeconds\": ${PHASE50_RANCHER_PROGRESS_DEADLINE_SECONDS},
      \"template\": {
        \"spec\": {
          \"containers\": [
            {
              ...
              \"startupProbe\": {
                ...
                \"timeoutSeconds\": ${PHASE50_RANCHER_STARTUP_TIMEOUT_SECONDS},
                \"periodSeconds\": ${PHASE50_RANCHER_STARTUP_PERIOD_SECONDS},
                \"failureThreshold\": ${PHASE50_RANCHER_STARTUP_FAILURE_THRESHOLD},
                \"successThreshold\": 1
              }
            }
          ]
...
require_rancher_origin_ready() {
  ...
  if ! "${kubectl_bin}" -n cattle-system rollout status deployment/rancher --timeout="${PHASE50_RANCHER_ROLLOUT_TIMEOUT}"; then
    rancher_debug_dump "rollout-timeout"
    fail_local_requirement \
      "rancher deployment did not become ready within ${PHASE50_RANCHER_ROLLOUT_TIMEOUT}; see ${PHASE50_RANCHER_DEBUG_LOG}"
  fi
  ...
  echo "[phase50] rancher origin checks passed"
}
...
require_gitea_golden_path() {
  ...
  "${kubectl_bin}" -n gitea patch deployment gitea --type merge \
    -p "{\"spec\":{\"progressDeadlineSeconds\":${PHASE50_GITEA_PROGRESS_DEADLINE_SECONDS}}}" >/dev/null 2>&1 || true
  ...
  if ! wait_for_gitea_rollout; then
    ...
    if [[ "${available_replicas}" -lt 1 || "${ready_replicas}" -lt 1 ]]; then
      fail_local_requirement \
        "gitea deployment did not become ready within ${PHASE50_GITEA_ROLLOUT_TIMEOUT} (attempts=${PHASE50_GITEA_ROLLOUT_ATTEMPTS}); see ${PHASE50_GITEA_DEBUG_LOG}"
    fi
    echo "[phase50] warning: gitea rollout did not fully settle, but deployment has ready replicas (ready=${ready_replicas} available=${available_replicas}); continuing with API health checks"
  fi
  ...
  echo "[phase50] gitea golden-path health checks passed"
...
require_local_domain_access() {
  ...
  PHASE50_LAST_HTTP_CODE=""

  resolved_ips="$(resolve_host_ips "${host}" | tr '\n' ' ' | xargs || true)"
  if [[ -z "${resolved_ips}" ]]; then
    echo "[phase50] ${host} does not resolve on this node" >&2
    return 1
  fi

  if [[ -n "${expected_vip}" ]]; then
    if ! printf '%s\n' ${resolved_ips} | grep -Fx "${expected_vip}" >/dev/null 2>&1; then
      echo "[phase50] ${host} resolves to [${resolved_ips}], expected ingress VIP ${expected_vip}" >&2
      return 1
    fi
  fi
  ...
  if [[ -z "${http_code}" || "${http_code}" = "000" ]]; then
    ...
    if [[ -n "${https_code}" && "${https_code}" != "000" ]]; then
      ...
      PHASE50_LAST_HTTP_CODE="${http_code:-${https_code}}"
      echo "[phase50] ${host} resolved but is not reachable over HTTP or HTTPS" >&2
      return 1
    fi
  fi
  PHASE50_LAST_HTTP_CODE="${http_code}"
  if ! printf '%s' "${http_code}" | grep -Eq "${PHASE50_DOMAIN_HTTP_SUCCESS_REGEX}"; then
    echo "[phase50] ${host} returned unexpected HTTP status ${http_code}" >&2
    return 2
  fi

  echo "[phase50] verified ${host} (ips=${resolved_ips}, scheme=${success_scheme}, http=${http_code})"
  return 0
}
...
check_domain_with_retry() {
  local host="${1:?host}"
  local expected_vip="${2:-}"
  local retries="${3:-${PHASE50_DOMAIN_CHECK_RETRIES}}"
  local delay="${PHASE50_DOMAIN_CHECK_DELAY}"
  local i=1
  local last_rc=1
  ...
  while [[ "${i}" -le "${retries}" ]]; do
    ...
    if [[ "${i}" -lt "${retries}" ]]; then
      echo "[phase50] domain check retry ${i}/${retries} for ${host} in ${delay}s" >&2
      sleep "${delay}"
    fi
    i="$((i + 1))"
  done
...
emit_dns_update_window_notice() {
  local vip="${1:-}"
  local retries="${2:-${PHASE50_DOMAIN_CHECK_RETRIES}}"
  local delay="${3:-${PHASE50_DOMAIN_CHECK_DELAY}}"
  ...
  if [[ -n "${vip}" ]]; then
    echo "[phase50] ingress VIP pinned at ${vip}"
  fi
  if (( total_seconds > 0 )); then
    echo "[phase50] waiting up to ${total_seconds}s (~${total_minutes}m) for DNS to converge; pull this log, update DNS to the pinned VIP above, and let bootstrap continue during that window"
  else
    echo "[phase50] DNS convergence wait is enabled; pull this log, update DNS to the pinned VIP above, and let bootstrap continue"
  fi
}

local_domain_retry_count() {
  if [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
    printf '%s' "${PHASE50_DOMAIN_CHECK_RETRIES}"
  else
    printf '%s' "${PHASE50_LOCAL_DOMAIN_CHECK_RETRIES:-3}"
  fi
}

gitea_local_domain_retry_count() {
  local retries="${PHASE50_GITEA_LOCAL_DOMAIN_CHECK_RETRIES:-}"
  if [[ -z "${retries}" ]]; then
    if [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
      retries="${PHASE50_DOMAIN_CHECK_RETRIES}"
    else
      retries=1
    fi
  fi
...
ensure_routing_frontdoor_service() {
  ...
  if [[ -z "${selector_yaml}" ]]; then
    echo "[phase50] warning: unable to derive ${workload_kind}/${workload_name} selector; using generic ingress-nginx controller selector" >&2
    selector_yaml=$'    app.kubernetes.io/name: ingress-nginx\n    app.kubernetes.io/component: controller'
  fi

  echo "[phase50] creating authoritative ingress front-door Service ${workload_ns}/${service_name} for ${workload_kind}/${workload_name}" >&2
...
metadata:
  ...
  labels:
    ...
    app.kubernetes.io/component: controller
    bootstrap.cluster.duck/managed-by: phase50
spec:
  type: ClusterIP
  selector:
${selector_yaml}
...
wait_for_routing_frontdoor_service() {
  local timeout="${PHASE50_INGRESS_FRONTDOOR_WAIT_TIMEOUT:-300}"
  local delay="${PHASE50_INGRESS_FRONTDOOR_WAIT_DELAY:-5}"
  local elapsed=0
  local found=""
  ...
  while [[ "${elapsed}" -le "${timeout}" ]]; do
    ...
    if [[ "${elapsed}" -eq 0 ]]; then
      echo "[phase50] waiting for authoritative ingress front-door Service" >&2
    fi
    sleep "${delay}"
...
argocd_debug_dump() {
  ...
  if [[ "${PHASE50_ARGOCD_DEBUG}" != "1" && "${PHASE50_ARGOCD_DEBUG}" != "true" ]]; then
    return 0
  fi

  mkdir -p "$(dirname "${PHASE50_ARGOCD_DEBUG_LOG}")" 2>/dev/null || true
  {
    echo ""
    echo "===== phase50 argocd debug @ ${ts} ====="
    ...
    echo "===== end phase50 argocd debug ====="
  } >>"${PHASE50_ARGOCD_DEBUG_LOG}" 2>&1
  bootstrap_diag_record_file_event \
    "argocd" \
    "argocd-debug" \
    "captured argocd diagnostics" \
    "${PHASE50_ARGOCD_DEBUG_LOG}" \
    "warning" \
    "http"
}
...
echo "[phase50] Authentik admin backup credentials are authoritative from ${BOOTSTRAP_SECRET_DIR} (source=${authentik_admin_source})"
...
PY
      ...
      if [[ -n "${authentik_admin_password_effective:-}" ]]; then
        ...
        echo "[phase50] Authentik admin backup credentials restored from OpenBao platform secret"
      fi
    fi
    if [[ -z "${authentik_bootstrap_token_effective:-}" ]]; then
      authentik_bootstrap_token_effective="$(
...
if [[ -n "${argocd_admin_password}" ]]; then
  ...
  echo "[phase50] ARGOCD_ADMIN_PASSWORD missing; Argo CD will use chart defaults." >&2
fi
...
if [[ "${PHASE50_RECONCILE_ONLY}" == "1" || "${PHASE50_RECONCILE_ONLY}" == "true" ]]; then
  echo "[phase50] reconcile-only mode: skipping Argo CD install playbook"
else
  echo "[phase50] installing Argo CD"
  ansible-playbook -i "${ANSIBLE_INVENTORY}" "${ANSIBLE_PLAYBOOK}" -e break_glass=true
fi

if [[ "${PHASE50_RECONCILE_ONLY}" == "1" || "${PHASE50_RECONCILE_ONLY}" == "true" ]]; then
  echo "[phase50] reconcile-only mode: skipping Gitea bootstrap and repo handoff"
else
  if [[ -z "${gitea_repo_password_effective}" ]]; then
  ...
  run_or_fail \
    ...
    "${PHASE50_GITEA_POSTGRESQL_SECRET_DELAY}"
  refresh_gitea_internal_service_url
  run_or_fail \
  ...
  if [[ "${PHASE50_MODE}" == "install" ]]; then
    echo "[phase50] minimal Argo CD + Gitea control pair installation complete"
    exit 0
  fi
  ...
  case "${trust_existing_gitea}" in
    ...
    *)
      ...
      if gitea_repo_has_readable_refs \
        ...
        echo "[phase50] trusting existing Gitea bootstrap repo with refs; skipping seed helper and remote reseed"
        echo "[phase50] Gitea repo selected -> ${gitea_repo_url_direct}"
        echo "[phase50] Argo CD repo source -> ${argocd_repo_url_direct}"
      else
        echo "[phase50] existing Gitea bootstrap repo has no refs; seeding from source repo"
        run_or_fail \
          "failed seeding Gitea bootstrap repo from source" \
          seed_gitea_bootstrap_repo \
          "${gitea_repo_password_effective}" \
  ...
  if [[ "${gitea_push_mirror_enabled_effective}" == "1" || "${gitea_push_mirror_enabled_effective}" == "true" || "${gitea_push_mirror_enabled_effective}" == "yes" ]]; then
    if [[ -n "${gitea_push_mirror_repo_url_effective}" ]] && [[ -n "${gitea_push_mirror_username_effective}" ]] && [[ -n "${gitea_push_mirror_token_effective}" ]]; then
      ...
      if ! configure_gitea_push_mirror \
        ...
        echo "[phase50] WARNING: failed configuring Gitea push mirror to GitHub; continuing bootstrap" >&2
      fi
    else
      echo "[phase50] Gitea push mirror requested but repo URL, username, or token is missing; skipping" >&2
    fi
  else
    echo "[phase50] Gitea push mirror disabled"
  fi
...
if [[ "${PHASE50_RECONCILE_ONLY}" == "1" || "${PHASE50_RECONCILE_ONLY}" == "true" ]]; then
  echo "[phase50] reconcile-only mode: skipping Argo CD repo handoff reconciliation"
else
...
echo "[phase50] enforcing Argo CD proxy mode (server.insecure=true)"
...
echo "[phase50] GitOps handoff complete: Argo CD now points at the seeded Gitea repo"
if [[ "${PHASE50_MODE}" == "handoff" ]]; then
  if [[ -d "${repo_root}/pods/ingress" ]]; then
    echo "[phase50] ensuring ingress front-door VIP before handoff exit"
    run_or_fail \
      "failed ensuring ingress front-door VIP before burn-the-ladder" \
      ensure_routing_frontdoor_vip
  fi
  echo "[phase50] post-handoff convergence checks deferred to the non-blocking post-handoff validation path"
  echo "[phase50] complete (GitOps handoff)"
  exit 0
fi
fi

echo "[phase50] running post-handoff convergence checks (warning-only=${PHASE50_WARNING_ONLY})"
...
if [[ -d "${repo_root}/pods/ingress" ]]; then
  ...
  echo "[phase50] ingress hostnames: external=${external_dns_bootstrap_hosts} local=${gitea_local_host},${argocd_local_host}"
  echo "[phase50] applying early-safe ingress resources"
  run_or_fail \
  ...
  for optional_ns in authentik headlamp homepage; do
    if "${kubectl_bin}" get namespace "${optional_ns}" >/dev/null 2>&1; then
      echo "[phase50] applying optional ingress for namespace ${optional_ns}"
      case "${optional_ns}" in
        authentik)
          run_or_fail \
      ...
      echo "[phase50] namespace ${optional_ns} not present yet; skipping optional ingress"
    fi
  done
  if [[ -d "${repo_root}/pods/ingress/observability-routing" ]]; then
    if "${kubectl_bin}" get namespace observability >/dev/null 2>&1; then
      echo "[phase50] applying pods/ingress/observability-routing"
      run_or_fail \
        "failed applying pods/ingress/observability-routing" \
        "${kubectl_bin}" apply -k "${repo_root}/pods/ingress/observability-routing"
    else
      echo "[phase50] observability namespace not present yet; skipping pods/ingress/observability-routing"
    fi
  fi
  run_or_fail \
    "failed applying kube-vip manifests" \
...
if [[ -d "${repo_root}/pods/ingress/external-dns" ]]; then
  if ensure_external_dns_cloudflare_secret; then
    ...
    echo "[phase50] skip: OpenBao source ingress/external-dns is unavailable"
  fi
else
  echo "[phase50] skip: missing ${repo_root}/pods/ingress/external-dns"
fi
...
if [[ "${domain_rc}" -ne 0 ]]; then
  ...
  if [[ "${domain_rc}" -eq 2 ]]; then
    if [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
      fail_local_requirement "${gitea_local_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} after waiting for Gitea readiness"
    fi
    echo "[phase50] warning: ${gitea_local_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} after waiting for Gitea readiness (non-strict local mode; possible stale DNS/VIP drift)" >&2
  elif [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
    fail_local_requirement "${gitea_local_host} failed DNS/HTTP validation after waiting for Gitea readiness"
  else
    echo "[phase50] warning: ${gitea_local_host} failed DNS/HTTP validation after waiting for Gitea readiness (non-strict local mode; possible stale DNS/VIP drift)" >&2
  fi
fi
domain_rc=0
...
if [[ "${domain_rc}" -ne 0 ]]; then
  if [[ "${domain_rc}" -eq 2 ]]; then
    if [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
      fail_local_requirement "${argocd_local_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} after retries"
    fi
    echo "[phase50] warning: ${argocd_local_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} after retries (non-strict local mode; possible stale DNS/VIP drift)" >&2
  elif [[ "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "1" || "${PHASE50_LOCAL_DOMAIN_CHECK_STRICT}" == "true" ]]; then
    fail_local_requirement "${argocd_local_host} failed DNS/HTTP validation after retries"
  else
    echo "[phase50] warning: ${argocd_local_host} failed DNS/HTTP validation after retries (non-strict local mode; possible stale DNS/VIP drift)" >&2
  fi
fi
...
if [[ "${external_dns_available}" -eq 1 ]]; then
  ...
  if [[ "${domain_rc}" -ne 0 ]]; then
    if [[ "${domain_rc}" -eq 2 ]]; then
      echo "[phase50] warning: ${gitea_external_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} (external warning-only mode; possible stale DNS/VIP drift)" >&2
    else
      echo "[phase50] warning: ${gitea_external_host} failed DNS/HTTP validation (external warning-only mode; possible stale DNS/VIP drift)" >&2
    fi
  fi
  domain_rc=0
  ...
  if [[ "${domain_rc}" -ne 0 ]]; then
    if [[ "${domain_rc}" -eq 2 ]]; then
      echo "[phase50] warning: ${argocd_external_host} returned unexpected HTTP status ${PHASE50_LAST_HTTP_CODE:-unknown} (external warning-only mode; possible stale DNS/VIP drift)" >&2
    else
      echo "[phase50] warning: ${argocd_external_host} failed DNS/HTTP validation (external warning-only mode; possible stale DNS/VIP drift)" >&2
    fi
  fi
else
  echo "[phase50] skipping external hostname validation because external-dns is not configured for this bootstrap run"
fi
