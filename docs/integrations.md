...
fork and recovery kit; do not paste generated token values into issues or
logs. Rerunning setup is the supported reconciliation path for incomplete
bootstrap resources.

## GitHub

**Purpose:** seed the fork into the cluster, retain an out-of-band mirror,
...
opinionated flow. It needs access to the fork and must be able to read, clone,
...
fork; after handoff, Gitea is the operational authority and GitHub is the
...
push the relevant fork changes: GitHub Actions runs the pushed repository
...
policy or credentials. If a node must be rebuilt, use the fork-owned
break-glass path rather than manually editing cluster identity on an existing
node.

## Safe validation order

1. Update the fork-owned `platform.yaml` and run `task setup:preflight`.
2. Commit and push the fork changes that GitHub workflows must consume.
