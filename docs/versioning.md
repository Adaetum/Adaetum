...
the required fork update or rebuild procedure. Users should validate their
fork with `task setup:preflight` and the relevant recovery path before relying
on a prerelease.
...
Adaetum is fork-first. A release does not promise an in-place upgrade for a
private fork or a running cluster. When a release changes a public contract,
the release notes must state the fork update, recovery/rebuild path, and any
