...
| `secret/apps/games/crafty/admin` | CSI-mounted desired Crafty administrator credential | Bootstrap seeds the native `admin` username and password in OpenBao. A path-scoped companion uses Crafty's supported API to replace the one-time generated password, verifies the OpenBao login, and observes in-place CSI password changes. Rotate the OpenBao field rather than changing the password only in Crafty's UI. |
