...
| `secret/apps/observability/ntfy` | direct `observability/ntfy-openbao` CSI files | Bootstrap provisions the administrator and the write-only Apprise publisher from these values. Rotate both in OpenBao, then restart ntfy and Apprise so their native auth database and destination URL converge together. |
...
| `secret/apps/games/crafty/admin` | Initial Crafty-native administrator recovery record | Crafty generates its normal `admin` username and random password on first start. A write-only, path-scoped companion promotes `default-creds.txt` into OpenBao, and Phase 99 includes it in the next recursive application-secret backup. A later password change in Crafty's UI does not update the initial credential file. |
...
| `secret/apps/gitea/registry` | `ansible/gitea-registry-creds` | Issue a replacement Gitea token, patch its public `host`, internal `push_host`, username, and token fields in OpenBao, and External Secrets rebuilds the Docker config used by both the bootstrap publisher and day-two runner. |
