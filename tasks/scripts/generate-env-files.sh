...
OPENBAO_TRANSIT_SEAL_MOUNT_PATH
OPENBAO_TRANSIT_SEAL_NAMESPACE
OPENBAO_TRANSIT_SEAL_TLS_SERVER_NAME
OPENBAO_TRANSIT_SEAL_CA_CERT_B64
...
# Optional external OpenBao Transit seal. The endpoint must be independent of
# the Adaetum OpenBao cluster; the token is a recovery-plane credential.
OPENBAO_TRANSIT_SEAL_ADDRESS=${OPENBAO_TRANSIT_SEAL_ADDRESS}
OPENBAO_TRANSIT_SEAL_TOKEN=${OPENBAO_TRANSIT_SEAL_TOKEN}
OPENBAO_TRANSIT_SEAL_KEY_NAME=${OPENBAO_TRANSIT_SEAL_KEY_NAME}
OPENBAO_TRANSIT_SEAL_MOUNT_PATH=${OPENBAO_TRANSIT_SEAL_MOUNT_PATH}
OPENBAO_TRANSIT_SEAL_NAMESPACE=${OPENBAO_TRANSIT_SEAL_NAMESPACE}
OPENBAO_TRANSIT_SEAL_TLS_SERVER_NAME=${OPENBAO_TRANSIT_SEAL_TLS_SERVER_NAME}
OPENBAO_TRANSIT_SEAL_CA_CERT_B64=${OPENBAO_TRANSIT_SEAL_CA_CERT_B64}
...
BG_HOST_AUTO=1
BG_HOSTNAME=
BG_TAILSCALE_TAGS="tag:\${TAILSCALE_CLUSTER_TAG:-cluster} tag:server tag:rocky10"
...
TAILSCALE_OPERATOR_OAUTH_CLIENT_ID="$(existing_value TAILSCALE_OPERATOR_OAUTH_CLIENT_ID)"
TAILSCALE_OPERATOR_OAUTH_CLIENT_SECRET="$(existing_value TAILSCALE_OPERATOR_OAUTH_CLIENT_SECRET)"
OPENBAO_TRANSIT_SEAL_ADDRESS="$(existing_value OPENBAO_TRANSIT_SEAL_ADDRESS)"
OPENBAO_TRANSIT_SEAL_TOKEN="$(existing_value OPENBAO_TRANSIT_SEAL_TOKEN)"
OPENBAO_TRANSIT_SEAL_KEY_NAME="$(existing_value OPENBAO_TRANSIT_SEAL_KEY_NAME)"
OPENBAO_TRANSIT_SEAL_MOUNT_PATH="$(existing_value OPENBAO_TRANSIT_SEAL_MOUNT_PATH)"
[ -n "${OPENBAO_TRANSIT_SEAL_MOUNT_PATH}" ] || OPENBAO_TRANSIT_SEAL_MOUNT_PATH="transit/"
OPENBAO_TRANSIT_SEAL_NAMESPACE="$(existing_value OPENBAO_TRANSIT_SEAL_NAMESPACE)"
OPENBAO_TRANSIT_SEAL_TLS_SERVER_NAME="$(existing_value OPENBAO_TRANSIT_SEAL_TLS_SERVER_NAME)"
OPENBAO_TRANSIT_SEAL_CA_CERT_B64="$(existing_value OPENBAO_TRANSIT_SEAL_CA_CERT_B64)"
...
[ -n "${default_ingress_public_hosts}" ] || default_ingress_public_hosts="$(printf '%s' "registry.${CLUSTER_DOMAIN},gitea.${CLUSTER_DOMAIN},argocd.${CLUSTER_DOMAIN},authentik.${CLUSTER_DOMAIN},headlamp.${CLUSTER_DOMAIN},home.${CLUSTER_DOMAIN},alertmanager.${CLUSTER_DOMAIN},grafana.${CLUSTER_DOMAIN},ntfy.${CLUSTER_DOMAIN},prometheus.${CLUSTER_DOMAIN},crafty.${CLUSTER_DOMAIN}")"
CLOUDFLARED_INGRESS_PUBLIC_HOSTS="${default_ingress_public_hosts}"
default_ingress_cleanup_hosts="$(printf '%s' "registry.${CLUSTER_DOMAIN},gitea.${CLUSTER_DOMAIN},argocd.${CLUSTER_DOMAIN},authentik.${CLUSTER_DOMAIN},headlamp.${CLUSTER_DOMAIN},home.${CLUSTER_DOMAIN},alertmanager.${CLUSTER_DOMAIN},grafana.${CLUSTER_DOMAIN},ntfy.${CLUSTER_DOMAIN},prometheus.${CLUSTER_DOMAIN},crafty.${CLUSTER_DOMAIN}")"
